...
<form method="post" action="/admin-failure-screenshot" target="_blank">
<label for="token2" style="margin-top:18px">Refresh token</label>
<input id="token2" name="token" type="password" autocomplete="off" required>
<button type="submit">View Last TPN Failure Screenshot</button>
</form>
...
@app.post("/admin-failure-screenshot")
def admin_failure_screenshot(token: str = Form(...)):
    expected = os.getenv("REFRESH_TOKEN")
    if not expected or token != expected:
        return HTMLResponse(admin_page("Invalid refresh token.", False), status_code=401)
